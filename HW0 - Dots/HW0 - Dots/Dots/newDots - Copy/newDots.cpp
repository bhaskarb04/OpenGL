...
//Draw Pacman
void draw_pacman(int x,int y,int dir)
{
	glBegin(GL_POLYGON);
		glColor3f(0.0,0.0,0.0);
...
void myDisplay(void)
{
    ...
    glEnd();
		...
		for(int i=0;i<d.paclist.size();i++)
		{
			...
			draw_pacman(d.paclist[i].x,d.paclist[i].y,direction);
			switch(direction)
			{
			case(LEFT):
...
int near_dot(int x,int y)
{
	...
	for(int i=0;i<d.paclist.size();i++)
	{
		if((x-d.paclist[i].x)*(x-d.paclist[i].x)+(y-d.paclist[i].y)*(y-d.paclist[i].y) < PACSIZE*PACSIZE)
		{
			return (2);
		}
	}
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("My first new attempt");
